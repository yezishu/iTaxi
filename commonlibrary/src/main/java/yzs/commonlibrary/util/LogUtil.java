...
public class LogUtil {
    ...
    private static boolean isPrint = AllUtilConfig.LogSwitch;
    private static boolean isDebug = false;

    public static final String TAG = LogUtil.class.getSimpleName();
    public static final String MSG = "log msg is null.";
    public static final String LOCAL_POWER_PATH = "";//FileAccessor.getExternalStorePath() + "/" + "doctor_log.txt";
    ...
    public static void e(String tag, String msg, Throwable throwable) {
        print(Log.ERROR, tag, msg, throwable);
    }
    ...
    private static void print(int mode, final String tag, String msg) {
        print(mode, tag, msg, null);
    }

    /*isPrint控制是否输出*/
    private static void print(int mode, final String tag, String msg, Throwable throwable) {
        if (!isPrint) {
            return;
        }
        // 设置打印的方法和加跳转链接
        Log.v(tag, setMethodAndLine(TextUtils.equals(TAG, tag)));
        if (msg == null) {
            return;
        }
        try {
     ...
     * 设置打印的方法和加跳转链接
     *
     * @param needStackAddOne 用于定位方法在栈中的位置
     * @return
     */
    private static String setMethodAndLine(boolean needStackAddOne) {
        String result = "";
        try {
            StackTraceElement thisMethodStack;
            int classPosition = 3; // 打印的类位置
            StackTraceElement[] stackTraceElements = (new Exception()).getStackTrace();
            for (int i = 0; i < stackTraceElements.length; i++) {
                if (!TextUtils.equals(stackTraceElements[i].getClassName(), LogUtil.class.getName())) {
                    classPosition = i;
                    break;
                }
            }
            thisMethodStack = stackTraceElements[classPosition];
//            if (needStackAddOne) {
//                thisMethodStack = stackTraceElements[classPosition + 1];
//            } else {
//                thisMethodStack = stackTraceElements[classPosition];
//            }
