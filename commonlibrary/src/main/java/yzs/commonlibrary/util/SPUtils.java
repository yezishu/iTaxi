...
/**
 * Created by Vamoose on 2015/9/15.
 */
public class SPUtils {
    public SPUtils() {
        throw new UnsupportedOperationException("cannot be instantiated"); /* cannot be instantiated */
    }
     ...
     * 加密保存
     */
    public static void saveEncode(Context context, String FILE_NAME, String key, String object) {
        SharedPreferences sp = context.getSharedPreferences(FILE_NAME, Context.MODE_PRIVATE);
        SharedPreferences.Editor editor = sp.edit();
        String tmp = EncryptionUtils.encode(object, AllUtilConfig.DES_Key);
     ...
     * 解密读取
     */
    public static String getDecode(Context context, String FILE_NAME, String key) {
        String result = "";
        SharedPreferences sp = context.getSharedPreferences(FILE_NAME, Context.MODE_PRIVATE);
        String tmp = sp.getString(key, "");
        if (tmp.length() > 0) {
            result = EncryptionUtils.decode(tmp, AllUtilConfig.DES_Key);
        }
        return result;
    }
    ...
    private static class SharedPreferencesCompat {
        ...
        private static Method findApplyMethod()// 反射查找apply的方法
        {
            try {
        ...
        public static void apply(SharedPreferences.Editor editor)// 如果找到则使用apply执行，否则使用commit
        {
            try {
